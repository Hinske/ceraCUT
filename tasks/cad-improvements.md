...
### 7.1 Polar-Koordinaten-Eingabe (`dist<winkel` / `@dist<winkel`) ✅ (V6.28)
- [x] `command-line.js` `CommandLine.parseInput()`: parst jetzt auch `dist<winkel` / `@dist<winkel` (command-line V1.5)
- **Datei:** `js/command-line.js`

### 7.2 PolylineTool Bogen-Modus (`A`) ist funktionslos ✅ (V6.33)
- [x] `segments[]` statt flachem `points[]` — Bogen-Segment braucht 2 Klicks (Durchgangspunkt + Endpunkt), Geometrie via `ArcTool._calcArcFrom3Points()` (jetzt statisch, gemeinsam genutzt)
- **Datei:** `js/drawing-tools.js` V2.15 (`PolylineTool`)

### 7.3 ArcTool: nur 3-Punkt-Modus ✅ (V6.33)
- [x] 7 neue Konstruktionsarten nach CircleTool-Submodus-Vorbild: SCE/SCA/SCL/SEA/SED/SER/CSE — alle Formeln vorab gegen bekannte geometrische Fälle verifiziert
- **Datei:** `js/drawing-tools.js` V2.15 (`ArcTool`)

### 7.4 RotateTool: kein `[Copy]`-Option ✅ (V6.28)
- [x] `acceptsOption`-Override für `C` (Copy) nachgerüstet, gleiches Muster wie ScaleTool (drawing-tools V2.13)
- **Datei:** `js/drawing-tools.js`

### 7.5 RectangleTool: kein Chamfer/Fillet (`C`/`F`), keine Rotation ✅ (V6.33)
- [x] Optionen `F`/`C`/`R` vor der 2. Ecke — nutzt `GeometryOps.filletPolyline()`/neues `chamferPolyline()` + `ModificationTool.rotatePoints()`
- **Datei:** `js/drawing-tools.js` V2.15 (`RectangleTool`), `js/geometry-ops.js` V2.6 (`chamferPolyline`)

### 7.6 LineTool: keine Fortsetzung vom letzten Endpunkt ✅ (V6.33)
- [x] Enter ohne Eingabe setzt vom letzten Linie/Bogen/Polylinie-Endpunkt fort (`DrawingToolManager._lastDrawEndpoint`, AutoCAD-LASTPOINT-Verhalten)
- **Datei:** `js/drawing-tools.js` V2.15

### 7.7 Selektion: kein `ALL`/`L` (Last) ✅ (V6.33)
- [x] `ModificationTool.acceptsOption`/`handleOption` akzeptiert jetzt `ALL` (alle Konturen) und `L` (zuletzt erzeugte Kontur, `app.lastCreatedContour`) zusätzlich zu `P`
- **Datei:** `js/drawing-tools.js` V2.15, `js/app.js` V6.21

### 7.8 Niedrige Priorität / bewusste Abweichung
- [x] BreakTool (`drawing-tools.js`) macht nur Zero-Gap-Splits, kein echter 2-Punkt-Gap wie AutoCAD-Default — für CAM bewusst sinnvoll (Wasserstrahl-Konturen sollen exakt geschlossen bleiben), keine Änderung vorgenommen
- [x] CircleTool `TTT` ist seit V6.31 vollständig implementiert (Newton-Raphson-Solver) — kein Action Item mehr
