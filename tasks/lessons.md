...
### [2026-07-14] R12-Export schrieb Layernamen mit Leerzeichen ("Layer 1") ungeprüft durch — AutoCAD 2017 stürzte beim Import ab, obwohl die Dateistruktur komplett valide war
- **Fehler:** User meldete: `FliesenMeyer_Logo_Entwurf6.dxf` (unser Export, V6.74) lässt AutoCAD beim Import abstürzen — trotz drei vorangegangener struktureller Fix-Runden (Owner-Handles V1.12, AC1009-Downgrade V1.13, EOF-Newline V1.14). Im Ordner: `acad.err` ("Unhandled Access Violation") und verwaiste `.dwl/.dwl2`-Locks als Absturzbeleg.
- **Root Cause:** Vollständige Datei-Forensik (Gruppencode-Paare, Bytes, Zahlenformate, POLYLINE/VERTEX/SEQEND-Sequenzen, Bulges, Degenerate-Geometrie) ergab: strukturell alles sauber. Der Unterschied zu seit Jahren funktionierenden R12-Referenzdateien (`Mercedes Logo.dxf`: `Layer_1`) lag im INHALT, nicht in der Struktur: Layername `Layer 1` mit Leerzeichen — in AC1009-Symbolnamen sind nur `A-Za-z0-9$-_` (max. 31 Zeichen) zulässig, R12-Reader parsen Namen historisch whitespace-getrennt. Der Name stammte unverändert aus der importierten Quelldatei (AC1027, wo Leerzeichen erlaubt sind) und wurde vom Writer ohne Format-Downgrade-Prüfung in LAYER-Tabelle und 6004 Entity-Gruppe-8-Felder geschrieben. Zusätzlich gefundene Landmine: Bild-Underlays wurden als IMAGE-Entity (existiert erst ab R14) mit XDATA einer nie registrierten APPID roh in die R12-Datei gepusht.
- **Regel:** Beim Export in ein ÄLTERES Format-Level reicht strukturelle Korrektheit nicht — alle aus der Quelle durchgereichten WERTE (Symbolnamen, Entity-Typen, Flags) müssen gegen die Regeln des ZIELformats validiert werden, nicht gegen die des Quellformats. Konkret: Namens-Sanitisierung mit konsistenter Original→Sanitisiert-Map (Tabelle und Entities müssen dieselbe Übersetzung nutzen, Kollisionen deduplizieren) und Entity-Typen, die es im Zielformat nicht gibt, überspringen statt roh schreiben. Bei "AutoCAD crasht"-Reports: funktionierende Fremd-/Altdateien im selben Ordner als Referenz heranziehen und gezielt die DIFFERENZEN suchen — die Absturzursache steckt in dem, was unsere Datei anders macht, nicht zwingend in dem, was formal gegen die Spec verstößt.
- **Betroffene Module:** `dxf-writer.js` (neu: `_sanitizeName`, POLYLINE-Dummy-Point, IMAGE-Skip, `_fmt`-NaN-Guard, V1.15)
